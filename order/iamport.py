...
def get_token():
    ...
    req = requests.post(url, data=access_data) #url에 접속해서 정보를 받아와라
    access_res = req.json() # request로 받아온 데이터를 json으로 해석하겠다
...
def find_transaction(order_id, *args, **kwargs): # 결제 완료 후 아이엠포트쪽에 실제로 결제된게 맞는지 확인해보는 작업
    access_token = get_token()
    if access_token:
